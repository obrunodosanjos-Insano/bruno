...
export const SpaceSection: React.FC<SpaceSectionProps> = ({ onOpenReservation }) => {
  const features = [
    {
      icon: <Wifi className="w-5 h-5 text-stone-900" />,
    ...
    {
      icon: <VolumeX className="w-5 h-5 text-stone-900" />,
    ...
    {
      icon: <Sun className="w-5 h-5 text-stone-900" />,
    ...
    {
      icon: <Coffee className="w-5 h-5 text-stone-900" />,
  ...
  return (
    <section id="espaco" className="py-12 sm:py-16 bg-[#fafafa] text-stone-900 border-b border-stone-200/60">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="text-center max-w-2xl mx-auto space-y-2">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-stone-100 border border-stone-200 text-stone-800 text-[10px] font-bold uppercase tracking-wider">
            <Laptop className="w-3.5 h-3.5 text-stone-900" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {features.map((feature, idx) => (
            <div 
              key={idx}
              className="bg-white p-5 rounded-2xl border border-stone-200/80 space-y-2"
            >
              <div className="w-9 h-9 rounded-xl bg-stone-100 flex items-center justify-center">
                {feature.icon}
              </div>
              <h3 className="text-xs font-bold text-stone-950 font-display">{feature.title}</h3>
              <p className="text-[11px] text-stone-600 leading-snug">{feature.description}</p>
            </div>
          ))}
        </div>

        {/* Space Zones Showcase */}
        <div className="bg-white text-stone-900 rounded-3xl p-6 sm:p-8 border border-stone-200/80 shadow-xs">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
            
            <div className="lg:col-span-7 space-y-4">
              <span className="text-[10px] font-bold text-stone-500 tracking-[0.2em] uppercase bg-stone-100 px-3 py-1 rounded-full border border-stone-200">
                RESERVA DE MESAS
              </span>
              <h3 className="text-xl sm:text-2xl font-bold font-display text-stone-950">
                Garanta seu lugar em nossa unidade no Pontão do Lago Sul
              </h3>

              <div className="space-y-2 pt-1">
                {zones.map((zone, i) => (
                  <div key={i} className="bg-[#fafafa] border border-stone-200 p-3.5 rounded-xl flex items-center justify-between">
                    <div>
                      <h4 className="text-xs font-bold text-stone-950 font-display">{zone.title}</h4>
                      <p className="text-[11px] text-stone-600">{zone.idealFor}</p>
                    </div>
                    <span className="text-[9px] font-extrabold uppercase px-2.5 py-0.5 rounded-full bg-stone-200 text-stone-800 shrink-0">
                      {zone.tag}
                    </span>
                  </div>
                ))}
              ...
              <div className="pt-2">
                <button
                  ...
                  className="px-6 py-3 rounded-full bg-stone-950 hover:bg-stone-800 text-white font-bold text-xs uppercase tracking-wider transition-all cursor-pointer flex items-center justify-center gap-2"
                >
